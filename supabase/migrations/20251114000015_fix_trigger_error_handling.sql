-- Fix trigger to not fail user creation if invitation processing fails
-- Instead, log errors and create Personal org as fallback
...
DECLARE
  ...
  workspace_name TEXT;
  invitation_processed BOOLEAN := FALSE;
BEGIN
  -- Get role IDs
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Failed to check invitations: %', SQLERRM;
    invited_org_id := NULL;
  END;

  -- Branch: Invited user vs Self-signup user
  IF invited_org_id IS NOT NULL THEN
    -- INVITED USER: Try to create workspace in invited organization
    BEGIN
      -- Verify organization exists
      IF EXISTS (SELECT 1 FROM public.organizations WHERE id = invited_org_id AND deleted_at IS NULL) THEN
        ...
        RAISE NOTICE 'Successfully processed invitation for user % to org %', NEW.email, invited_org_id;
      ELSE
        RAISE WARNING 'Invited organization % does not exist', invited_org_id;
      END IF;

    EXCEPTION WHEN OTHERS THEN
      -- Log error but don't fail - will create Personal org instead
      RAISE WARNING 'Failed to provision invited user workspace: %. Creating Personal org instead.', SQLERRM;
      invitation_processed := FALSE;
    END;
  END IF;

  -- Create Personal org if: (1) not invited OR (2) invitation processing failed
  IF NOT invitation_processed THEN
    BEGIN
      ...
      RAISE NOTICE 'Created Personal org for user %', NEW.email;

    EXCEPTION WHEN OTHERS THEN
      -- This is critical - re-raise to fail user creation
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Smart auto-provision with robust error handling. Creates workspace in invited org for invited users, or Personal org+workspace for self-signup users. Falls back to Personal org if invitation processing fails.';
